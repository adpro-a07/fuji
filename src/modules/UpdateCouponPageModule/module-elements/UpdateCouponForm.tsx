...
export default function UpdateCouponForm({ coupon, couponId }: { coupon: CouponFormValues; couponId: string }) {
  ...
  const updateCoupon = async (values: CouponFormValues): Promise<StructuredResponse<unknown>> => {
    try {
      const payload = {
        ...values,
        validUntil: formatDate(values.validUntil, "yyyy-MM-dd"),
      }
      const response = await put(`/api/v1/coupons/${couponId}`, payload, {
        toAuthBackend: false,
        isAuthorized: true,
      })
      ...
      if (navigator.onLine && !(error as { type?: string }).type) {
  ...
  return (
    <Form {...form}>
      <form onSubmit={onSubmit} className="mx-auto max-w-xl space-y-6 rounded-xl border p-6 shadow-sm">
        <h1 className="text-center text-2xl font-bold">Update Coupon</h1>
        <FormField
